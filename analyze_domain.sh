...
# --- Domain Analysis Script (v5) ---
# This script performs a multi-layered analysis of a given domain,
# covering DNS resolution, IP ownership, DNS provider, and HTTP service details.
# v5 translates the output to Traditional Chinese.
...
# Smarter root domain detection
get_root_domain() {
    local domain=$1
    # This is a simplified approach using a public suffix list idea.
    # It handles common cases like .com, .io, .co.uk, .com.tw
    local suffixes="com.tw co.uk com.au"
    local suffix_match=false
    for suffix in $suffixes; do
        if [[ "$domain" == *."$suffix" ]]; then
            echo "$domain" | rev | cut -d. -f1-3 | rev
            suffix_match=true
            break
        fi
    done
    if [ "$suffix_match" = false ]; then
...
main() {
  ...
  # 4. DNS Provider
  print_section "3. DNS 服務商 (DNS Provider)"
  local root_domain
  root_domain=$(get_root_domain "$DOMAIN")

  echo "    - 推測的根網域: $root_domain"
  local ns_records
  ns_records=$(dig +short NS "$root_domain")
  
  if [ -z "$ns_records" ]; then
    echo "    無法確定此根網域的 NS 紀錄。"
  else
    echo "    - 名稱伺服器 (NS Records):"
    while read -r ns; do
      echo "        - $ns"
    done <<< "$ns_records"
    ...
    echo -e "\n    * ${GREEN}分析:${NC} 此網域的 DNS 服務由 **$dns_provider** 管理。"
  fi

  # 5. HTTP Service Analysis
  print_section "4. HTTP 服務分析 (HTTP Service Analysis)"
  local curl_output
  # Use -w to append effective URL at the end, -D - to dump headers to stdout
  ...
  if [ -z "$curl_output" ]; then
    ...
    local final_url
    final_url=$(echo "$curl_output" | tail -n 1)
    local all_headers
    all_headers=$(echo "$curl_output" | sed '$d') # Remove last line (the url)

    # Get the last set of headers
    local final_headers
    final_headers=$(echo "$all_headers" | awk '/^HTTP\// {buffer=""} {buffer=buffer $0 "\n"} END {printf "%s", buffer}')
    
    local status_code
    status_code=$(echo "$final_headers" | head -n 1 | awk '{for (i=2; i<=NF; i++) printf $i " "; print ""}' | xargs)
    local server_header
    server_header=$(echo "$final_headers" | grep -i '^server:' | awk '{$1=""; print $0}' | xargs)
    local via_header
    via_header=$(echo "$final_headers" | grep -i '^via:' | awk '{$1=""; print $0}' | xargs)
    local xcache_header
    xcache_header=$(echo "$final_headers" | grep -i '^x-cache:' | awk '{$1=""; print $0}' | xargs)
    ...
    if echo "$server_header" | grep -qi "cloudfront" || echo "$via_header" | grep -qi "cloudfront"; then
        http_provider="AWS CloudFront"
    elif echo "$server_header" | grep -qi "cloudflare"; then
        http_provider="Cloudflare"
    elif echo "$server_header" | grep -qi "amazons3"; then
        http_provider="Amazon S3"
    elif [ ! -z "$server_header" ]; then
        http_provider="$server_header"
    fi
