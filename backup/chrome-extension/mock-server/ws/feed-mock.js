...
// 由「宣告式 spec」建出 mode:'mock' 的 feed WS mock。給 .ws.json 模組用（由「新增 Mock WS」產生）。
// 每個 topic 每 intervalMs 推一次 payloadFile 的內容——每 tick 重讀檔（改 json 即時生效）。
// 動態資料（seq 遞增、價格模擬）表達不了 → 手寫 .js 用 createFeedMock 的 make(i)。
//   spec = { path, mode:'mock', topics: { <topic>: { intervalMs, payloadFile } } }
export function buildDeclarativeFeed(spec, { dir }) {
  if (!spec?.path) throw new Error('宣告式 ws feed 缺少 path')
  const topics = {}
  for (const [topic, cfg] of Object.entries(spec.topics ?? {})) {
    if (!cfg?.payloadFile) throw new Error(`ws feed ${spec.path} 的 topic '${topic}' 缺少 payloadFile`)
    const filePath = path.resolve(dir, cfg.payloadFile)
    topics[topic] = {
      intervalMs: cfg.intervalMs ?? 1000,
      make: () => {
        try {
          return JSON.parse(readFileSync(filePath, 'utf8')) // 每 tick 重讀，改檔即變
        } catch (err) {
          // 讀檔失敗不讓 interval 爆掉，推一個看得見的錯誤幀
          return { topic, error: `payloadFile 讀取失敗: ${err.message}` }
        }
      },
    }
  }
  return createFeedMock({ path: spec.path, topics })
}
