...
const USER_ROWS = [
  {
    ...
    // First Deposit Time 取代 First Trading Time（api doc B12）。兩個都留著：
    // NexU 只會看到 firstDepositTime，其他品牌只會看到 firstTradingTime，
    // 所以兩個都給值才驗得出「該藏的藏了、該顯示的顯示了」。
    firstDepositTime: 1756000000000,
  ...
  },
  {
    ...
    firstDepositTime: null, // 未入金 → 表格該顯示 '-'
    kycStatus: 0,
    note: '',
    roleStatus: 'DIRECT_USER',
    upgradable: true,
...
const LINK_ROWS = [
  {
    ...
    referredUsers: 42,
    tradedUsers: 17,
    depositedUsers: 20 // 取代 tradedUsers（api doc B11）
  },
  {
    referralCode: 'NEXU02',
...
//
// ⚠️ firstDeposit 只有 data，沒有 periodTotal / gain —— 這是共享行為，8081 實測三條 series
...
export default function register(app, { defaultApiDomain } = {}) {
  ...
  const PERFORMANCE_SUMMARY = {
    ...
    // 8081 實測：共享版 summary 有回這三個（都是 SelectedSummary.vue 在讀的），
    // 文件沒列。null / 過去的 timestamp 都代表「可以改」。
    availableUpdateCommissionTime: null,
    availableUpdateSelfRebateTime: null,
    commissionCreationTime: 1748000000000
  }
