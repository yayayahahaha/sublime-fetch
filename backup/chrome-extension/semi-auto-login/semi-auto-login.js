...
function createBlock(data = {}, index) {
  ...
  const opts = [
    ...
    new window.Option('BTZO', 'btzo'),
    new window.Option('Lmex', 'lmex'),
    new window.Option('Traiex', 'traiex'),
    new window.Option('NVX', 'nvx'),
    new window.Option('Fedhabit', 'fedhabit'),
    new window.Option('Bitqik', 'bitqik'),
    new window.Option('Bitkub', 'bitkub'),
    new window.Option('Autotrader', 'autotrader'),
  ]
...
document.getElementById('semi-auto-login-addBtn').addEventListener('click', () => {
  list.push({ email: '', password: '', twofa: '', brand: '', sort: list.length + 1, port: '' })
  saveList()
  renderAllBlocks()
})
